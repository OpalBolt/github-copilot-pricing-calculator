...
A static, two-tool site for comparing model prices. A landing hub links to two
calculators: GitHub Copilot and Cortecs. Each is a sortable table of models with
an estimated cost for a token mix you set.
...
The two tools fetch data differently.
...
generate_html.py            # Renders the hub + both tools
...
```bash
nix develop
python generate_html.py            # fetch both data sources, render the hub + both tools
