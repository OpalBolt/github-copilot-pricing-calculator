...
things that CAN break silently and the JS depends on:
  ...
  1. The template -> JS data contract: every paygo row carries numeric
     ...
     the JS to fill, a sub-row class + drill-toggle, and both have
     .est-cost / .runs / .tokens cells plus a compare button. No row carries
     data-tier (tier is global from radios).
  2. The formulas as a Python reference: recompute blended price and
     ...
     effective price. Subscription Est. cost is effective $/1M × tokens per run
     (same per-run semantics as paygo) — the plan price lives on monthlyCost.
...
def extract_rows(html: str) -> list[dict]:
    ...
    for tr in re.findall(r"<tr[^>]*data-provider-id=.*?</tr>", html, re.S):
        ...
        rows.append({
            ...
            "sub_row": 'class="sub-row"' in tr,
            "drill_toggle": 'class="drill-toggle"' in tr,
        })
    return rows
...
def main() -> int:
    ...
    for r in subs:
        ...
        assert r["cmp_btn"]
        assert r["sub_row"] and r["drill_toggle"], f"{r['model']} must be an expandable sub-row"
    ...
    # Est. cost = effective $/1M × tokens-per-run (same per-run semantics as paygo, so the
    # relative-cost bar is apples-to-apples); runs = months the budget buys;
    ...
    # Default budget $10 < $18/mo → no surplus: Est. cost = 0.32092 × 1M tokens/1M ≈ $0.32,
    # monthlyCost stays $18 (drill-down), runs 0.56 mo, tokens = 0.56 × monthly cap.
    lite = {"monthly_usd": 18, "weekly_credits": 10000}
    glm52_sub = {"mult_input": 6.9, "mult_cached": 1.7, "mult_output": 24.0}
    price, monthly, runs, tokens, surplus = sub_math(glm52_sub, lite)
    assert math.isclose(price, 0.3209237875, rel_tol=1e-9), price
    assert math.isclose(monthly, 18.0, rel_tol=1e-9), monthly
    cost = price * TOKENS_PER_RUN / 1_000_000
    ...
    _, monthly_s, runs_s, tokens_s, surplus_s = sub_math(glm52_sub, lite, budget=100)
    assert monthly_s == 18.0
    ...
    # ── Subscription drill-down contract (fair-cost slice) ──
    # Sub rows are expandable (Cortecs pattern): sub-row class + toggle in the
    # template, a colspan=11 detail row, and the JS pieces that must exist.
    assert HTML.count('class="sub-row"') == len(subs)
    assert HTML.count('class="drill-toggle"') == len(subs)
    assert 'colspan="11"' in HTML and 'drilldown-row' in HTML
    assert "tr.sub-row.expanded td" in HTML.replace("\n", " ") or "tr.sub-row.expanded td" in HTML
    assert "out.monthlyCost = tier.monthly_usd" in HTML
    assert "priceFor(cfg) * cfg.tokensPerRun / 1_000_000" in HTML
    assert "collapseDrilldowns" in HTML and "onSubRowClick" in HTML
    assert "Est. cost shown is effective $/1M at your token mix, not the monthly plan price." in HTML
